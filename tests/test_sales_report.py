...
def replay_fixture():
    ...
    for player, state in enumerate(after):
        state["action"] = {"market": [["SELL", "WHEAT", 99]]}
        state["observation"]["step"] = 1
        state["observation"]["farms"][player]["money"] += revenue
...
def test_costs_include_successful_purchases_hiring_and_land():
    ...
    assert p0["breakdown"] == {
        "BUY_SEED:WHEAT": 2 * seed_cost,
        "HIRE:HIRE": hire_cost,
        "BUY_LAND:BUY_LAND": land_cost,
    }
