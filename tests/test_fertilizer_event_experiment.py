...
FERTILIZER opportunity cost at the exact day it would be consumed, and that a
partial late-only fertilizer schedule can beat both current boolean choices.
"""
...
def _score_case(initial_fertilizer_stock, fertilizer_sales_per_day):
    ...
    # monotonically soften its future price unless the candidate consumes it.
    baseline = Production()
    ...
    # Exact opportunity cost of consuming one FERTILIZER on each event day,
    # against the same baseline supply path.  Positive means cash forgone.
    opportunity = {}
    for when in EVENTS:
        consume = Production()
    ...
    rows.sort(key=lambda row: (row["market_cash"], -len(row["fertilize_days"]), row["fertilize_days"]), reverse=True)
    best = rows[0]
    ...
    return {
        ...
        "events": list(EVENTS),
        "fertilizer_opportunity_cost": opportunity,
...
def test_late_only_fertilizer_plan_can_beat_boolean_commitment():
    ...
    # proof of the economic case discussed in planner review: fertilizer is
    # expensive early, animal supply pushes its value down, and only later
    # WHEAT cycles justify consuming it instead of selling it.
    suffixes = {tuple(EVENTS[index:]) for index in range(1, len(EVENTS))}
    found = None
    searched = []
    for supply in (1, 2, 3, 4, 5, 6):
        for stock in range(-500, 501, 5):
            case = _score_case(stock, supply)
            plan = tuple(case["best_event_plan"]["fertilize_days"])
            if plan in suffixes and case["gain_over_boolean"] > 0:
                found = case
                break
        searched.append({"supply": supply, "found": found is not None})
    ...
    costs = found["fertilizer_opportunity_cost"]
    selected = found["best_event_plan"]["fertilize_days"]
    omitted = [day for day in EVENTS if day not in selected]
    # The exact dated opportunity cost should have fallen by the time the first
    # selected late event occurs; otherwise the suffix result would be suspect.
    assert omitted and selected
    assert costs[str(selected[0])] < costs[str(omitted[0])]
