...
def test_trace_first_day_four_wheat_successor_candidate():
    """Diagnose the first harvested WHEAT tile whose successor must materialize."""
    env = make("kaggriculture", configuration=configuration(1), debug=False)
    agent = make_agent(END_DAY, seed=1)
    cells = {
        name: cell.cell_contents
    ...
    for step in range(int(env.configuration.episodeSteps) - 1):
        ...
        if obs.day == 4 and scheduled_wheat:
            empty = [
                position for position in sorted(scheduled_wheat)
                if farm["tiles"][position[1]][position[0]] is None
            ]
            if empty:
                position = empty[0]
                frozen = set(planner_state.get("frozen_positions", ()))
                ...
                for task in replanned:
                    task.mandatory = True
                target_tasks = [task for task in replanned if task.position == position]
                starts = [tuple(farm["farmer"]), *map(tuple, farm["hands"])]
                ...
                budget = max(0, 24 - obs.hour)
                candidate, unassigned = build_rolling_queues(
                    replanned,
                    starts,
                ...
                raise AssertionError({
                    ...
                    "target_tasks": [
                        (task.actions, dict(task.needs), task.mandatory)
                        for task in target_tasks
                    ],
                    "candidate_unassigned": [
                        (task.position, task.actions, dict(task.needs))
                        for task in unassigned
                    ],
                    "candidate_lengths": [len(plan.queue) for plan in candidate],
                    "incumbent_lengths": [
                        len(plan.queue) for plan in planner_state.get("plans", ())
                    ],
                })
    ...
    raise AssertionError("simulation ended before a Day-4 WHEAT successor appeared")
