...
def test_build_opening_targets_matches_fixed_portfolio():
    ...
    assert OPENING_COUNTS == {"WHEAT": 19, "COW": 2, "SHEEP": 2, "GOOSE": 2}
...
def test_fixed_opening_governs_days_zero_and_one():
    governs = make_opening_controller()
    targets = {}
    assert OPENING_HANDOFF_DAY == 2
    assert governs(make_obs(0), targets, NW_POSITIONS) is True
    assert governs(make_obs(1), targets, NW_POSITIONS) is True
    assert governs(make_obs(2), targets, NW_POSITIONS) is False


def test_opening_hands_off_to_planner_on_day_two_permanently():
    governs = make_opening_controller()
    targets = {}
...
def test_handoff_preserves_portfolio_without_scripted_conversions():
    governs = make_opening_controller()
    targets = {}
    governs(make_obs(0), targets, NW_POSITIONS)
...
def test_animals_occupy_the_six_nearest_shed_tiles_even_with_shuffled_input():
    targets = build_opening_targets(list(reversed(NW_POSITIONS)))
    animals = {p for p, target in targets.items() if target[0] != "WHEAT"}
    assert animals == {(4, 4), (3, 4), (4, 3), (2, 4), (3, 3), (4, 2)}
