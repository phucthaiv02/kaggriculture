...
def wheat_tile(planted_day=0, yield_units=1):
    return {"kind": "PLANT", "crop": "WHEAT", "planted_day": planted_day, "yield_units": yield_units, "watered_today": True}
...
def test_day_two_conversion_still_creates_third_cow_and_sheep_before_handoff():
    ...
    closest = sorted(wheat_positions, key=lambda p: (abs(p[0] - 4) + abs(p[1] - 4), -p[1], -p[0]))
    growing = {closest[0]: wheat_tile(), closest[1]: wheat_tile()}
    ...
    assert Counter(value[0] for value in targets.values()) == Counter({"MELON": 12, "WHEAT": 7, "COW": 3, "SHEEP": 3})
