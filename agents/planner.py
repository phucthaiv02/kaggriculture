...
def evaluate_targets(market, baseline, candidates, labor=None, position=(4, 4)):
    ...
    def scoped(flow):
        ...
        for field in ("sales", "inputs", "visits"):
            getattr(result, field).update((d, value) for d, value in getattr(flow, field).items() if market.day <= d <= end)
        return result
...
def _choose(market, baseline, candidates, counts, labor=None, position=(4, 4), *, current=None, audit=None, decision_log=None, decision_step=None):
    results = evaluate_targets(market, baseline, candidates, labor, position)
    if audit is not None:
        audit.extend(results)
    scored = [(r.profit, r) for r in results if r.profit > 0]
    if not scored:
